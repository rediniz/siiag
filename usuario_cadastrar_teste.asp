	...
	else
	{
		...
		document.getElementById('f_matricula').focus();
			...
			$(document).ready(function(){
				...
				$("#f_matricula").focus().autocomplete("autocompletar_matricula.asp",{
				  ...
				  $("#f_nome").focus().autocomplete("autocompletar_nome.asp",{
					minChars: 1 //Numero minimo de caracteres para aparecer a lista
				  , matchContains: true //Aparecer somente os que tem relação ao valor digitado
				  , scrollHeight: 220 //Altura da lista dos nomes
				  , selectFirst: true  //Vim o primeiro da lista selecionado
...
if cStr(request("f_salvar")) = "1" then
	...
	if rs.eof then
		...
		sql = sql & " VALUES ('" & matricula & "','0', '" & nome & "','" & cargo & "','" & nascimento & "','" & jornada & "'," & gerente_aux & "," & ativo_aux & ",'" & imagem_gravar & "','" & coordenacao & "','" & co_unidade & "','" & admissao & "','" & inicio & "') "
		'response.Write(sql)
...
<a href="#dialog" name="modal"><INPUT type="button"  value="Adicionar foto"  class="myButton" style="padding:3px; font-size:13px; height:25px; width:108px;" tabindex="12" ></a>
            ...
            <tr>
                ...
                <input  type='checkbox' name='f_ativo' id='f_ativo' <%if ativo = "true" then%> checked="checked" <%end if%>  tabindex='9'/>
			  </td>
