...
Query = " SELECT A.*, B.DE_TIPO, C.NO_AREA, ARS.NU_AR  FROM TB_PROTOCOLO_ATUAL A  LEFT JOIN TB_PROTOCOLO_TIPO B ON A.CO_TIPO = B.CO_TIPO LEFT JOIN TB_PROTOCOLO_AREAS C ON A.CO_AREA = C.CO_AREA"
  	Query = query & " LEFT JOIN [TB_PROTOCOLO_ARS] ARS ON (A.NO_PROTOCOLO  = ARS.NO_PROTOCOLO AND A.CO_TIPO = ARS.CO_TIPO	)	"
	cont = 0
  ...
  'response.End()
	...
	if not pc.eof then
		...
		do while (not PC.eof and Abs(pc.AbsolutePage - pag) < numPag )
			...
			response.write "<td id='numOficio' value="&numeroOficio&">" & numeroOficio & " / " & AAProtocolo & "</td>"
			...
			response.write "<td>" & nuAR & "</td>"			
			response.write "<td id='codArea' value="&codigoArea&" style='display:none;'>" & codigoArea & "</td>"
			response.write "<td>" & nomeCodigoArea & "</td>"
			response.write "<td>" & FormataData(dataEnvio) & "</td>"
			response.write "<td style='text-align:left'>" & FormataFrase(nomeResponsavel) & "</td>"
			response.write "<td style='text-align:left'>" & conteudo & "</td>"
			response.write "<td style='text-align:left'>" & destino & "</td>"
		...
		Response.Write "<table>"
		Response.Write "<tr>"
		Response.Write "<td width='100%'>NÃO FORAM LOCALIZADOS DADOS PARA O FILTRO SELECIONADO</td>"
		Response.Write "</tr>"
		Response.Write "</table>"
	end if
