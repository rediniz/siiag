...
<%
	...
	conexao.Open = "Provider=MSDASQL;Driver={SQL Server};Server="&SERVIDOR_DO_BANCO&";Database=GIFUG;UID="&USUARIO_DO_BANCO&";PWD="&SENHA_DO_BANCO&";" 
	consulta = conexao.execute(sql_co_responsavel_matricula)
	co_responsavel_matricula = consulta("CO_MATRICULA")
	Set conexao = nothing
	
	' Efetua a conexão com o banco SIIAG
	Set conexao = Server.CreateObject("ADODB.Connection")
	...
	Set queryCodTipoProtocolo = conexao.execute(sql_codTipoProtocolo)
	...
	for i=1 to qtd_arquivo
		...
		Set query3 = conexao.execute(sql3)
