...
if cStr(request("f_salvar")) = "1" then
	...
	gerente = request("f_gerentesubstituto")
	...
	if rs.eof then
		...
		sql = sql & " VALUES ('" & matricula & "','0', '" & nome & "','" & cargo & "','" & jornada & "','" & gerente & "','" & ativo & "','" & coordenacao & "','" & admissao & "','" & inicio & "') "
		response.Write(sql)
	else
...
%>
    <select name="f_coordenacao" id="f_coordenacao"  class="form-select" style=" width:340px;max-width:340px;min-width:340px;" >
        ...
        <option value="">Selecione a Coordenação</option>
            ...
            DO UNTIL ds.eof
                ...
                if cstr(request("coordenacao"))	= cstr(ds("co_gs")) then
