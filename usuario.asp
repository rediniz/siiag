...
{     
	...
	document.getElementById('f_sexo').removeAttribute('disabled');
	document.getElementById('f_sexo').className = 'form_bloqueado';
	document.getElementById('f_sexo').style.background = '#FFFFFF';
	...
	if cStr(request("flg_salvar")) = "1" then
		...
		if not rs.eof then
				if arquivo <> "" then
					...
					sql = sql & " SET NO_NOME = '" & nome & "', CO_CARGO = '" & cargo & "', DT_NASCIMENTO = '" & nascimento & "', NU_JORNADA = '" & left(jornada,1) & "', CO_GS = '" & coordenacao & "', IC_GERENTE = " & gerente_aux & ", IC_ATIVO = " & ativo_aux & ", DT_ADMISSAO = '" & admissao & "', DT_INICIO_GIFUG = '" & inicio & "', IM_FOTO = '" & imagem_gravar & "', IC_NAO_EXIBIR_ANIV = " & nao_aniver_aux & ", sexo = '"& sexo &"'  WHERE CO_MATRICULA='" & left(matricula,7) & "'"
				else
					sql= "UPDATE TB_USUARIOS "
					sql = sql & " SET NO_NOME = '" & nome & "', CO_CARGO = '" & cargo & "', DT_NASCIMENTO = '" & nascimento & "', NU_JORNADA = '" & left(jornada,1) & "', CO_GS = '" & coordenacao & "', IC_GERENTE = " & gerente_aux & ", IC_ATIVO = " & ativo_aux & ", DT_ADMISSAO = '" & admissao & "', DT_INICIO_GIFUG = '" & inicio & "', IC_NAO_EXIBIR_ANIV = " & nao_aniver_aux & ", sexo = '"& sexo &"' WHERE CO_MATRICULA='" & left(matricula,7) & "'"
				end if
				arquivo = ""
				'response.write sql
		end if
...
%>
      ...
      <div class="titulosModulos" ><span >USUÁRIO</span> </div>
        <div class="usuariodiv"  style="border:#000000 0px solid;" >
	      ...
	      <table border="0" width="90%">
           ...
           <br/>
            <table border="0" width="980px" cellpadding="3px" cellspacing="0px" style="table-layout: fixed;" >
            <tr>
                <td >
              ...
              <legend class="style2 style3">Informações do Usu&aacute;rio</legend>
                ...
                <table border="0" width="950" align="center" cellspacing="0" style="margin-top:10px;">
				<tr>
         		<td width="100">
                    <label >Matr&iacute;cula: </label>
                    </td>
                 <td colspan="1" width="100">
                     <input class='form-field' name='matricula' id='matricula' type='text'  size='8' maxlength='7'  value= '<%=matricula %>' style="width:70px;"  readonly="readonly"/>
                     -
                    ...
                    <td colspan="5" >
                     <input class='form-field' name='co_unidade' id='co_unidade' type='text'  size='15' maxlength='7'  value= '<%=co_unidade %> - <%=sg_unidade %> ' disabled="disabled" style="width:115px;" />
  ...
  <input class='form-field' name='nome' id='nome' type='text'  size='60' maxlength='100' value='<%= nome%>'  readonly="readonly" style="max-width:350px; min-width:350px;" />
                 ...
                 <td width="150" rowspan="6" colspan="2"  align="center" valign="top" style="max-width:150px;">
 ...
 <tr>
                    ...
                    <td >
                      <input class='campo_bloqueado' name='nascimento' id='nascimento' type='text'  size='10' maxlength='10' value='<%=nascimento %>' disabled='disabled' OnKeyPress='return(FormatarData(this,event))'  onblur='return(ValidarData(this,this.value))' style=' width:115px;text-align:center'       />
                  </td>
              ...
              <%if perfil_acesso_admin_siiag or (request.Cookies("ic_gerente_siiag") or request.Cookies("co_usuario_siiag") = matricula ) then%>
                <tr>
                    ...
                    <td  colspan="1"  >
                      <input class='campo_bloqueado' name='admissao' id='admissao' type='text'   maxlength='10'  value='<%= admissao %>' disabled="disabled" OnKeyPress="return(FormatarData(this,event))"  onblur="return(ValidarData(this,this.value))" style=' width:115px;text-align:center' />
                  </td>                
                    <td >
                    <label >Início na Unid:</label>
                  </td>
                    <td  colspan="3"  >
                      <input class='campo_bloqueado' name='inicio' id='inicio' type='text'  size='15' maxlength='10' value='<%= inicio %>' disabled="disabled" OnKeyPress="return(FormatarData(this,event))"  onblur="return(ValidarData(this,this.value))" style=' width:115px;text-align:center'/>
                  </td>
                ...
                <tr>
                    ...
                    <td   colspan="1" width="130" ><label id="lblSubst_2" style="display:<%=esconde_lbl%>;">
                ...
                <tr>
                    <td colspan="8">
                    <label >Retirar o nome do empregado das listas de aniversariantes: </label>
...
Sub ComboSexo()
%>
    <select name="f_sexo" id="f_sexo"  class="form-select" style="width:128px;" tabindex="3"  disabled="disabled" >

        <option value="">Selecione o sexo</option>
        <option value="M" <%if sexo = "M" then %> selected="selected" <%end if%> >Masculino</option>
        <option value="F" <%if sexo = "F" then %> selected="selected" <%end if%> >Feminino</option>        
    </select>
<%
End Sub
%>
