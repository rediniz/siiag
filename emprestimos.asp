...
<script language="javascript" type="text/javascript">
	...
	$(document).ready(function() {
		...
		$(document).on("click","#solicitar", function(){
			...
			var qt;
			if($("#check_termo").prop("checked") == false){
	...
	$(function() {
		...
		$( "#adicionarRecurso" ).click(function() {
			...
			$(".linha").each(function(){
				codAdicionado = $(this).find("#recurso").val();
				if(codAdicionado == codRecurso){
			...
			//=================================================================================
				...
				$(".linha").each(function(){
					recursos = recursos + $(this).find("#recurso").val()+":"+$(this).find("#quantidade").html()+";";
				});
				
				//recursos = recursos.substring(0, recursos.length-1);
				
//				window.location.assign("eventos_verificar_disp_recurso.asp?local="+local+"&periodo_inicio=" + periodo_inicio + "&periodo_fim=" + periodo_fim + "&horario_inicio=" + horario_inicio +"&horario_fim="+horario_fim+"&recursos="+codRecurso+"&tipo=2");
				
				$.ajax({
				type: "GET",
				url: "eventos_verificar_disp_recurso.asp?local="+local+"&periodo_inicio=" + periodo_inicio + "&periodo_fim=" + periodo_fim + "&horario_inicio=" + horario_inicio +"&horario_fim="+horario_fim+"&recursos="+codRecurso+"&tipo=2",
				success: function (data) {
			...
			$("#verificar_disp").on("click", function(){
				...
				//window.location.assign("eventos_verificar_disp_recurso.asp?local="+local+"&periodo_inicio=" + periodo_inicio + "&periodo_fim=" + periodo_fim + "&horario_inicio=" + horario_inicio +"&horario_fim="+horario_fim+"&recursos="+recursos+"&tipo=2");
				
				$.ajax({
				type: "GET",
				url: "eventos_verificar_disp_recurso.asp?local="+local+"&periodo_inicio=" + periodo_inicio + "&periodo_fim=" + periodo_fim + "&horario_inicio=" + horario_inicio +"&horario_fim="+horario_fim+"&recursos="+recursos+"&tipo=2",
				success: function (data) {
					if(data == ""){
						alert("Disponível.");
					}else{
						data = data.substring(0, data.length-1);
						var mensagem = "Os seguintes recursos não estão disponíveis: \n "+data;
						alert(mensagem);
					}
				},	
				error: function (data) {
                        ...
                        <fieldset class="testess" style=" border-color: #FFCC66; border:#FFCC66 1px solid; border-radius:4px; margin-bottom:10px">
                            ...
                            <p class="paragrafoteste">     
                                ...
                                <table>
                                    ...
                                    <!--<button id="verificar_disp" class="myButton" style="width:180px; margin-left:0px; font-size:12px; padding-left:4px;padding-right:4px" type="button">Verificar Disponiblidade</button>-->
                                    ...
                                    <td><br /><input type="checkbox" id="check_termo" /> Li e concordo com o <a href='termo.asp'>Termo de Responsabilidade</a></td></tr>
                                    <tr>
                                        <td>
                                        </td>
                                        <td>
