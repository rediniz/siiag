...
do while not leitura.eof
    ...
    <tr>
    	<td colspan="3">
    		<hr style="height:2px; color:#a0a0a0" />
        </td>
    </tr>
    <tr class="bodyt">
    	<td class="datat">
        	<%=response.Write("&nbsp;" & leitura("DIA") & "/" & mes)%>
        </td>
        <td class="nomet">
            <%response.Write(ucase(leitura("NOME")))%>
        </td>
        <td class="gerenciat">
        	<%response.Write(leitura("GERENCIA"))%>
        </td>
    </tr>
	<%
	leitura.movenext
...
<script language="javascript">
	alert("Para evitar impressões em formatos indesejados, após selecionar a impressora, clique em 'Preferências' (caso esteja usando o Internet Explorer, ou 'Propriedades' se estiver no Firefox) e mude a 'Orientação' para paisagem.");
	window.print();
