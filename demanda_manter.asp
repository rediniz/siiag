...
<script>
	$(document).ready(function() {
		...
		$('#bt_add_usuario').on('click',function(){
		
			$('#linha_incluir_usuario').show();
	
		});

		$('#bt_cancel_usuario').on('click',function(){
			$('#linha_incluir_usuario').hide();
		});

		$('#bt_save_usuario').on('click',function(){
				var co_demanda = $('#co_demanda').val();
				var co_matricula = $('#co_usuario_responsavel').val();
				var co_tipo_responsavel = $('#co_tipo_responsavel').val();

				var confirmacao = confirm('Confirma a inclusão do usuário?');
				
				if (confirmacao){
					$.ajax({
						method: "POST",
						contentType: "application/x-www-form-urlencoded; charset=utf-8; code=65001", 
						url: "demanda_usuario_salvar.asp?co_demanda="+co_demanda + '&co_matricula=' + co_matricula + '&co_tipo_responsavel=' + co_tipo_responsavel , // ENDEREÇO DA PÁGINA PARA ONDE OS DADOS SERÃO PASSADOS
						dataType: "html", // TIPO DE DADOS DE RETORNO
						//timeout: 5000, // TEMPO LIMITE PARA AGUARDAR RESPOSTA DA REQUISIÇÃO
						beforeSend: function() { // ANTES DA REQUISIÇÃO
					...
					//window.location.assign("demanda_usuario_salvar.asp?co_demanda="+co_demanda + '&co_matricula=' + co_matricula + '&co_tipo_responsavel=' + co_tipo_responsavel);
				}
				
		});
		
		$('.bt_delete_usuario').on('click',function(){
				var co_sequencial = $(this).parent().find("#co_seq_usuario").val();

				var confirmacao = confirm('Confirma a exclusão do usuário?');
				
				if (confirmacao){
					$.ajax({
						method: "POST",
						contentType: "application/x-www-form-urlencoded; charset=utf-8; code=65001", 
						url: "demanda_usuario_excluir.asp?co_sequencial="+co_sequencial , // ENDEREÇO DA PÁGINA PARA ONDE OS DADOS SERÃO PASSADOS
						dataType: "html", // TIPO DE DADOS DE RETORNO
						//timeout: 5000, // TEMPO LIMITE PARA AGUARDAR RESPOSTA DA REQUISIÇÃO
						beforeSend: function() { // ANTES DA REQUISIÇÃO
    ...
    <div class="container">
      <div class="conteudo2">
        ...
        <div id='submenu'>
		 ...
		 <% if perfil_acesso_admin_siiag and 2=3 then  %>
        ...
        <table border="0" align="center" width="750px;">
          <tr>
            <td colspan="2" ><label> <strong> Dados do Solicitante: </strong> </label></td>
          </tr>
          <tr>
            <td width="180" colspan="1" >
            	<label>Matr&iacute;cula/Nome: </label>
            </td>
            <td width="560" colspan="1" ><label><%=rs("SOLICITACAO_MATRICULA") %> - <%=rs("NO_NOME") %></label>
            </td>
          </tr>
          <tr>
            <td width="180"><label>Coordena&ccedil;&atilde;o Demandante: </label></td>
			<td><label><%=rs("NO_COORD_DEMANDANTE") %></label></td>
          </tr>
          <tr>
            <td width="180" colspan="2"><br/><hr width="700px" align="left" /></td>
		   </tr>

		</table>
         <table border="0" align="center" width="750px;">          
          <tr>
            <td colspan="2" ><label> <strong> Dados da Demanda: </strong></label></td>
          </tr>
          <tr>
            <td width="179"><label>Código / Tipo:</label></td>
            <td width="561" style="margin-top:5px; max-width:250px; min-width:250px; "  name="nome" id="nomeSolicitante" type="text" size="43" maxlength="100"><input type="hidden" name="tipo_demanda" id="tipo_demanda" value="<%=rs("tipo_demanda")%>"  /><label><%= rs("co_demanda") %> - <%= rs("NO_TIPO") %></label></td>
          </tr>

          <tr>
            <td width="179"><label>Nome do Aplicativo:</label></td>
            <td width="561" style="margin-top:5px; max-width:50px; min-width:50px; "  name="nome" id="codigoDemanda" type="text" size="3" maxlength="3"><input type="hidden" name="codigo_demanda" id="codigo_demanda" value="<%=rs("co_demanda")%>"  /><label><%= rs("NOME") %></label> </td>
          </tr>
          <tr>
            <td width="179"  valign="top"><label>Descrição: </label></td>
            <td width="561" colspan="2"><textarea readonly="readonly"  style="margin-top:5px; max-width:500px; min-width:500px; max-height:100px;min-height:100px;" class="form-field" name="descricao" id="descricao" size="60" rows="5" cols="50"><%=rs("DESCRICAO") %></textarea>
            </td>
          </tr>
		  <tr>
          <td></td>
		  ...
		  <tr>
          <td colspan="3"><hr width="700px" align="left" /></td>
          </tr>

          <tr>
            <td colspan="3" ><label> <strong>Arquivos anexados:</strong></label></td>
          </tr>
          <tr>
          <td ></td>
          <td colspan="1">
          ...
          <tr>
            <td colspan="1" ><label> <strong>Usuários envolvidos:</strong></label></td>
            <%if perfil_acesso_admin_siiag then %>
    	        <td colspan="2" style="text-align:left;cursor:pointer;" id="bt_add_usuario" ><img src="imagens/add_16x16.png"  />adicionar usuário</td>
			<%else%>
        ...
        <% if perfil_acesso_admin_siiag then %>
                ...
                <table border="0" align="center" width="750px">
                  <tr>
                    <td width="179"><label><strong>Situa&ccedil;&atilde;o:</strong> </label></td>
                    <td width="" name="nome" id="nomeSolicitante" type="text" size="43" maxlength="100" >
                  ...
                  <tr>
               		<td valign="top"><label><strong>Motivo/Observação:</strong></label></td>
                      <td>
                ...
                <table border="0" width="750px" class="tabela_5" align="center" <% if rs("co_situacao") <> 2 then response.write " style='display:none;' " %>>
                  <tr>
                    <td colspan="4">&nbsp;</td>
                  </tr>
                  <tr>
		...
		<table name='tabela' class="tabela" style=" width:750px;" align="center">
        <tr>
        	<td width='15%'>Data</td>
...
<%
Sub Monta_Envolvidos() %>
	<table name='tabela' class="tabela" style=" width:95%;" border="1">
        <tr>
        	<td width='60%'>Usuário</td>
            <td width='30%'>Responsabilidade</td>
            <td width='10%'></td>
        </tr> 
        <%
			query = "SELECT  * from VW_USUARIOS_RESPONSAVEL where co_demanda = " & demanda & " order by co_tipo_responsavel "
			set ss=Server.CreateObject("ADODB.RecordSet")
			ss.Open query, dados_sys
			do while not ss.eof
		%>
        <tr>
        	<td width='60%' style=" text-align:left;"><%=UCASE(ss("CO_MATRICULA_RESPONSAVEL"))%> - <%=ss("NO_RESPONSAVEL")%></td>
            <td width='30%'><%=ss("NO_TIPO_RESPONSAVEL")%></td>
            <td width='10%' style=" text-align:center;">
            <% if perfil_acesso_admin_siiag then %>
                <img src='imagens\delete_16x16.png' class='bt_delete_usuario' title='Excluir usuário' style='cursor:pointer;' /> 
                <input type='hidden' name='co_seq_usuario' id='co_seq_usuario' value='<%=ss("sequencial")%>'/>
            <%end if%>
            </td>
        </tr> 
		<%
		...
		%>
        <tr id="linha_incluir_usuario" style="display:none;">
        	<td width='60%' style=" text-align:left;"><%Combo_Usuario%></td>
            <td width='30%'><%Combo_Tipo_Resp%></td>
            <td width='10%' style=" text-align:center;">
            <img src='imagens\save_16x16.png'   id='bt_save_usuario' title='Salvar usuário' style='cursor:pointer;' /> 
            <img src='imagens\delete_16x16.png' id='bt_cancel_usuario' title='Cancela operação' style='cursor:pointer;' /> 
            </td>
        </tr> 
		</table>	     
...
Sub Combo_Usuario()
%>
    <select id='co_usuario_responsavel' name='co_usuario_responsavel' class="form-select" style="width:300px;">
          <option value="">Selecione...</option>
          <%	
            SQL = "SELECT NO_NOME, CO_MATRICULA FROM [VW_USUARIOS] WHERE IC_ATIVO = '1' "
            SQL = SQL + " AND CO_MATRICULA <> '' AND LETRA in ('C','E') "
            ...
            rs_usuario.Open SQL, dados_sys, 3, , 32
            
            do while(rs_usuario.eof = false)
                
                nome = rs_usuario("NO_NOME")
                response.Write "<option " 
                response.Write " value='"& rs_usuario("CO_MATRICULA") &"'>" & ucase(nome) & "</option>"
                rs_usuario.MoveNext
...
Sub Combo_Tipo_Resp()
%>
    <select id='co_tipo_responsavel' name='co_tipo_responsavel' class="form-select" style="width:150px;">
          <option value="">Selecione...</option>
          <%	
            SQL = "SELECT * FROM [TB_DEMANDAS_TIPOS_RESPONSAVEL] ORDER BY CO_TIPO_RESPONSAVEL "
            ...
            do while(rs_usuario.eof = false)
                ...
                nome = rs_usuario("NO_TIPO_RESPONSAVEL")
                response.Write "<option " 
                response.Write " value='"& rs_usuario("CO_TIPO_RESPONSAVEL") &"'>" & ucase(nome) & "</option>"
                rs_usuario.MoveNext
