	...
	if(chk_cargo_todos <> "1") then
		for i = 1 to request("qt_cargos")
			if request("chk_cargo_" & i) <> "" then
				cont = cont + 1
				campos(cont) = " (CO_CARGO = '" & request("chk_cargo_" & i) & "')"
	...
	if(chk_coordenacao_todos <> "1") then
		for i = 1 to request("qt_coordenacoes")
			if request("chk_coord_" & i) <> "" then
				cont = cont + 1
				campos(cont) = " (CO_GS = '" & request("chk_coord_" & i) & "')"
...
%>
        <tr>
        <td style="display:<%=campo_matricula_exibir%>; font-family: Verdana, Arial, Helvetica, sans-serif; font-size:14px;"><%=ucase(RS("CO_MATRICULA"))%></td>
        <td align="left" style="display:<%=campo_nome_exibir%>;  font-family: Verdana, Arial, Helvetica, sans-serif; font-size:14px;"><%=capitalize(RS("NO_NOME"))%></td>
        <td style="display:<%=campo_cargo_exibir%>;  font-family: Verdana, Arial, Helvetica, sans-serif; font-size:14px;"><%=capitalize(RS("NO_CARGO"))%></td>
        <td style="display:<%=campo_coordenacao_exibir%>;  font-family: Verdana, Arial, Helvetica, sans-serif; font-size:14px;"><%=RS("NO_GS")%></td>
        <td style="display:<%=campo_assinatura_exibir%>; font-family: Verdana, Arial, Helvetica, sans-serif; font-size:14px;"></td>
        </tr>
<%
		rs.movenext
	Loop
