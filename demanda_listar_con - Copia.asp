...
<script>
	$(document).ready(function() {
		...
		$('.urgencia').on('click',function(){
		
		if(matricula == "C022903" || matricula == "C053498" || matricula =="C124937" || matricula =="E712237"){
			
			//var demanda = $(this).parent().parent().find("#demanda").val();
			//alert(demanda);
			//window.location.assign("demanda_urgencia.asp?demanda="+demanda+"");
			
			
		}
		else{
			alert("Você não tem permissão para solicitar urgência");
...
<%
	...
	strQuery = "SELECT DEMANDAS.CO_DEMANDA, DEMANDAS.SOLICITACAO_DATA, USUARIOS.NO_NOME, DEMANDAS_TIPOS.NO_TIPO, APLICATIVOS.NOME, DEMANDAS.ORDEM_PRIORIDADE, DEMANDAS_SITUACOES.DE_SITUACAO, DEMANDAS.CONCLUSAO_DATA_PREVISTA, B.* "
