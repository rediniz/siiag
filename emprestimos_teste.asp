...
<script>
	...
	$(document).ready(function() {
		...
		// Remover linha da tabela e pegar valores
		$('#tabela').on('click', '#X', function(){
		
		// Pegando os valores da linha
		var recurso = $(this).parent().parent().find(".recurso").html();
		var quantidade = $(this).parent().parent().find(".quantidade").html();
		
		// Testando os valores obtidos
		alert("Recurso: " + recurso + ", Quantidade: " + quantidade);
		...
		//código para remover do banco
		
		//remove a linha da tabela (porém não será preciso porque ao remover do banco, a página será atualizada)
...
%>
          ...
          <label>Per&iacute;odo:</label>
           <input  name="periodo_de" id="periodo_de" class="datepicker" type="text" size="10" maxlength="10"  value="<% Response.Write(Date) %>"/>

          &nbsp;&agrave;&nbsp;
           <input  name="periodo_a" id="periodo_a" class="datepicker" type="text" size="10" maxlength="10"  value="<% Response.Write(Date) %>"/>
