...
$(function() {
 	...
 	$( "#incluirPublico" ).click(function() {
	   ...
	   if (quantidade == 0 || quantidade == ""){
	    ...
	    if(periodo_inicio == "" || periodo_fim == "" || horario_inicio == "00:00" || horario_fim == "00:00"){
			alert("Informe dia e horário antes de adicionar um recurso.");
			return false;
		}
		...
		$.ajax({
		type: "GET",
		url: "eventos_verificar_disp_recurso.asp?local="+local+"&periodo_inicio=" + periodo_inicio + "&periodo_fim=" + periodo_fim + "&horario_inicio=" + horario_inicio +"&horario_fim="+horario_fim+"&recursos="+codigo+"&tipo=2",
		success: function (data) {
			
			//Retorno vazio, recurso disponível
			if(data == ""){
			...
			//Recurso não disponível
				...
				var mensagem = "O recurso não está disponível para esse dia e horário.";
				
				alert(mensagem);
			}
		},	
		error: function (data) {
			alert("Erro ao verificar disponibilidade do recurso.");
		},	
		cache:false
		});
		//================================================================================================
...
$("#salvar").click(function(){
	...
	if($("#quantidade_recurso").val() > 0 && $("#check_termo").prop("checked") == false){
				alert("Você deve aceitar os termos antes de fazer uma solicitação.");	
				return false;
	}
	...
	$("#verificar_disp").on("click", function(){
		...
		if(local == "" || periodo_inicio == "" || periodo_fim == "" || horario_inicio == "" || horario_fim == ""){
			
			alert("Para verificar a disponibilidade, informe local, período e horário.");
			return false;
		}
		...
		$.ajax({
			...
			method: "GET",
			url: "eventos_verificar_disp.asp?local="+local+"&periodo_inicio=" + periodo_inicio + "&periodo_fim=" + periodo_fim + "&horario_inicio=" + horario_inicio +"&horario_fim="+horario_fim+"&tipo=1",
			success: function (data) {
				if(data == "1"){

					$("#status_disp").html("DISPONÍVEL");
					$("#status_disp").css("color", "#090");
					$("#status_disp").show();	
				}else{

					$("#status_disp").html("NÃO DISPONÍVEL");
					$("#status_disp").css("color", "#C00");
					$("#status_disp").show();
				}
				},
			error: function() {
            ...
            <table width="915" height="135" border="0" cellpadding="0" cellspacing="5">
              ...
              <tr>
                ...
                <td colspan="4"><select class="form-select" name="iniciativa" id="iniciativa">
                    <option value=""></option>
				<%
                query = "SELECT CO_GS, NO_GS FROM VW_GS WHERE co_unidade = " & request.Cookies("co_usuario_unidade_siiag") &" and ic_ativo = 1 order by no_gs"
                set ds=Server.CreateObject("ADODB.RecordSet")
              ...
              <tr>
                ...
                <td colspan="4"><input id="horario_inicio" name="horario_inicio" class="ui-mask ui-spinner-input" autocomplete="off" role="spinbutton" size="4" value="00:00" style="height:24px"/>
                  <input id="horario_fim" name="horario_fim" class="ui-mask ui-spinner-input" autocomplete="off" role="spinbutton" size="4" value="00:00" style="height:24px"/><button type="button" id="verificar_disp" class="myButton" style="width:180px; margin-left:8px; font-size:12px">Verificar Disponibilidade</button><span id = "status_disp" style="font-size:16px; margin-left:8px"></span></td>
                  <td colspan="2"></td>
              </tr>
              <tr>
          ...
          <div id="tabss">
            ...
            <div id="recursos">
              ...
              <input name="quantidade_recurso" id="quantidade_recurso" class="form-field" type="text" size="2" maxlength="5" style="font-size:12px"/>
                <!--<button id="verifica_recurso" class="myButton" style="font-size:12px">Verificar Disponibilidade</button>-->
              ...
              <div class="tabela" align="center">
               ...
               <!--#ALTERADO EM 14/05/2016 (removido o imprimir termo e colocado o check)-->
               <br />
               <div align="center">
 			   	<input type="checkbox" id="check_termo" /> Li e concordo com o <a href='termo.asp' style="color:#00F" target="new">Termo de Responsabilidade</a>
 			   </div>
               <!------------------------------------------->
            </div>
            <div id="material">
