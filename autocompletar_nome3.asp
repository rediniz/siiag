...
<%
	...
	if Request.QueryString("responsavel") = "sim" then
				...
				'sql = "SELECT USUARIO.CO_MATRICULA, USUARIO.NO_NOME, USUARIO.DT_NASCIMENTO, USUARIO.NO_GS, CARGO.NO_CARGO, GS.NO_GS FROM VW_USUARIOS AS USUARIO JOIN TB_CARGOS AS CARGO ON USUARIO.CO_CARGO = CARGO.CO_CARGO where usuario.no_nome like '%" & valor & "%' and ic_ativo = 1 and co_unidade ="&request.Cookies("co_usuario_unidade_siiag")&" and co_cargo in (1, 2, 3, 4, 5, 6, 7, 8) ORDER BY USUARIO.NO_NOME;"
	
		sql = "SELECT * FROM TB_USUARIOS where NO_NOME like '%" & valor & "%' AND IC_ATIVO = 1 and co_cargo in (1, 2, 3, 4, 5, 6, 7, 8) and co_unidade ="&request.Cookies("co_usuario_unidade_siiag")&" ORDER BY NO_NOME;"
		
	else
	
		'sql = "SELECT USUARIO.CO_MATRICULA, USUARIO.NO_NOME, USUARIO.DT_NASCIMENTO, USUARIO.NO_GS, CARGO.NO_CARGO, GS.NO_GS FROM VW_USUARIOS AS USUARIO JOIN TB_CARGOS AS CARGO ON USUARIO.CO_CARGO = CARGO.CO_CARGO where usuario.no_nome like '%" & valor & "%' and ic_ativo = 1 and co_unidade ="&request.Cookies("co_usuario_unidade_siiag")&" ORDER BY USUARIO.NO_NOME;" 
...
End Select
	...
	End Select				
	    ...
	    output = output & "{""nome"":""" & query("no_nome") & """,""matricula"":""" & query("co_matricula") & """,""nascimento"":""" & query("dt_nascimento") &  """,""cargo"":""" & cargo & """,""value"":""" & query("no_nome") & """},"
