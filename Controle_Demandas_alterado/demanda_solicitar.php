...
<body>
	<div class="teste">
		<!--fundo azul do meio-->
		<?	include 'topo.php'; //incluindo o topo, onde há também a inclusão da conexao		?>
        <div class="corposite">
            <!--corpo-->
            <div class="container">
                <div class="conteudo2">
                <!-- <div class="titulosModulos" ><span >ESTOQUE</span> </div>  -->
                    ...
                    <fieldset class="testess"style=" border-color: #FFCC66; border:#FFCC66 1px solid; border-radius:4px;">
                        ...
                        <table border="0">
                            <form action="demanda_solicitar_con.php" method="post">
                                <tr> <!-- abre tipos -->
                                    ...
                                    <td><select class="form-select" id="tipoDemanda" name="tipoDemanda" style="max-width:213px; min-width:213px;">
                                        <option value="0">Escolha o Tipo de Demanda</option>
                                            <?  
                                                abreConexao();//chama a função que abre a conexao
                                        
                                                $query = "SELECT * FROM TB_DEMANDAS_TIPOS";
                                                $resultado = mssql_query($query,$conexao);
                                
                                                while($linha = mssql_fetch_array($resultado)) {
                                                    echo "<option value=".$linha['CO_TIPO'].">".$linha['NO_TIPO']."</option>";
                                                }
                                ...
                                <tr>
                                            ...
                                            <option value="0">Selecione o Aplicativo</option>
                                                <?  
                                                    abreConexao(); //chama a função que abre a conexao
                                                    $query = "select CO_APLICATIVO, NOME from TB_APLICATIVOS ORDER BY NOME"; //variavel que aloca a consulta
                                                    $resultado = mssql_query($query,$conexao); // variavel que recebe o resultado da consulta
                                
                                                    while($linha = mssql_fetch_array($resultado)) {
                                                        echo "<option name=".$linha['CO_APLICATIVO']." value=".$linha['CO_APLICATIVO'].">".$linha['NOME']."</option>";
                                                    }
                                                    //mssql_close($conexao); //fechando a conexao aberta acima
                                                ?>
                                            </select></td>
            ...
            <? include 'rodape.php'  //importando o rodape ?>
        </div>
    </div>
    <!--teste-->
</body>
