...
<%
	...
	data = day(date()) & "/" & month(date()) & "/" & year(date()) & " " & time()
		aa_protocolo = objUpload("aa_protocolo").Value
	...
	'sql_co_responsavel_matricula = "SELECT CO_MATRICULA FROM TB_USUARIOS WHERE NO_NOME = '"& responsavelProtocolo & "'"
	...
	'Pega a pasta padrão na qual serão gravados os arquivos.
	QueryPasta  = "SELECT * FROM TB_PROTOCOLO_ARQUIVOS_LOCAL_GRA"
	Set rsPasta = objConn.execute(QueryPasta)
	Pasta       = replace(rsPasta("LOCAL_GRAVACAO_ARQUIVOS"),"\","/")
	set rsPasta = nothing								
	...
	for i=1 to qtd_arquivo
		...
		If Not ScriptObject.FolderExists(Pasta & prot &"/") Then
			pastaProtocolo = ScriptObject.CreateFolder(Pasta & prot &"/")
		Else
			pastaProtocolo = Pasta & prot &"/"
		End If
		
		cont = 0
		arquivo_novo = arquivo.FileName
		while ScriptObject.FileExists(pastaProtocolo & "\" & arquivo_novo)
			cont = cont + 1

'			If ScriptObject.FileExists(pastaProtocolo & "\" & arquivo.FileName) Then
				'n_aux = split(arquivo.FileName, arquivo.FileExt)
			arquivo_novo = left(arquivo.FileName, len(arquivo.FileName)-4) & "_" & cont & right(arquivo.FileName, 4)
				'arquivo.FileName = n_aux & "_1" & "." &  arquivo.FileExt
'			End If
'			arquivo.FileName
		wend
		arquivo.FileName = arquivo_novo
		arquivo.SaveAs(pastaProtocolo & "\" & arquivo.FileName)


		
		' Insere as informações no banco
		sql3 = "INSERT INTO TB_PROTOCOLO_ARQUIVOS (NO_PROTOCOLO,AA_PROTOCOLO,CO_TIPO,NO_ARQUIVO,SG_EXTENSAO,VR_TAMANHO_KB,CO_CRIADOR,DH_CRIACAO,CO_AREA) VALUES ("&	numeroProtocolo&",'" & aa_protocolo & "'," &codTipoProtocolo& ",'" & arquivo.FileName & "','" & extensao & "',"& tamanho_arquivo & ",'" & request.Cookies("co_usuario_siiag") & "','" & data & "'," & areaProtocolo & ")"
