...
<%
	...
	strQuery = "SELECT  " 
	strQuery = strQuery &  " EVENTOS.CO_EVENTO, EVENTOS.DE_EVENTO,  EVENTOS.DH_INICIO, EVENTOS.DH_FIM,  EVENTOS.DH_CRIACAO, EVENTOS.CO_CRIADOR, EVENTOS.QT_PARTICIPANTES,TIPO.DE_TIPO_EVENTO,SITUACOES.DE_SITUACAO,GS.NO_GS,LOCAL.DE_LOCAL, EVENTOS.QT_PARTICIPANTES_PRESENTES "
	...
	strQuery = strQuery &   " WHERE ('" & date() & "' > DH_FIM AND QT_PARTICIPANTES_PRESENTES IS  NOT NULL AND EVENTOS.CO_SITUACAO IN (2) AND convert(varchar(10), PREST_CONTAS_DATA,103) = '" & date() & "' AND LEFT(PREST_CONTAS_MATRICULA,1) <> 'S' )  "	
	...
	%>
             <table  >
              <tr>
                ...
                <td width="6%" >Qtde<br>Presente</td>
              </tr>
	<%
...
<%
			...
			response.write "<td id='' >" & RS("QT_PARTICIPANTES_PRESENTES") & "</td>"
			response.write "</tr>"
			rs.movenext
		LOOP
	
		Controla_Pagina_prest()
...
<%
	...
	Sub Controla_Pagina_prest()
		 ...
		 '****CONTROLE DE PÁGINA
		  ...
		  Response.Write "onClick='JavaScript:window.navigate(""?" & rsSubst("PagPrest", CStr(1), "") & """);' value='&lt;&lt;'>"
	
				
		  Response.Write "<input type='button' class='myButton'"
		  If (Pag <= numPag) Then Response.Write " disabled "
		  Response.Write "onClick='JavaScript:window.navigate(""?" & rsSubst("PagPrest", CStr(Pag - numPag), "") & """);' value='&lt;'>"
		  ...
		  Response.Write "<input type='button' class='myButton'"
		  If ((Pag + numPag - 1) >= Ult) Then Response.Write " disabled "
		  WriteLn "onClick='JavaScript:window.navigate(""?" & rsSubst("PagPrest", 1 + (CStr(Round(Ult / numPag + 0.5, 0))-1)*numPag, "") & """);' value='&gt;&gt;'>"
