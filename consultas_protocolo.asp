...
'dim cont
	...
	'response.Write("consultar_base_antiga:" & consultar_base_antiga)
	
	Pag = cint(request("pag"))
	if Pag = 0 then Pag = 1
   'todos = Request("todos")

	if consultar_base_antiga  then
		Query = " SELECT A.*, B.DE_TIPO, C.NO_AREA, ARS.NU_AR  FROM TB_PROTOCOLO_ANTIGO A  LEFT JOIN TB_PROTOCOLO_TIPO B ON A.CO_TIPO = 	B.CO_TIPO LEFT JOIN TB_PROTOCOLO_AREAS C ON A.CO_AREA = C.CO_AREA"
		...
		Query = " SELECT A.*, B.DE_TIPO, C.NO_AREA, ARS.NU_AR, USUARIOS.CO_GS  FROM TB_PROTOCOLO_ATUAL A  LEFT JOIN TB_PROTOCOLO_TIPO B ON A.CO_TIPO = 	B.CO_TIPO LEFT JOIN TB_PROTOCOLO_AREAS C ON A.CO_AREA = C.CO_AREA"
		Query = query & " LEFT JOIN [TB_PROTOCOLO_ARS] ARS ON (A.NO_PROTOCOLO  = ARS.NO_PROTOCOLO AND A.CO_TIPO = ARS.CO_TIPO AND A.AA_PROTOCOLO=ARS.AA_PROTOCOLO)	"
		Query = query & " LEFT JOIN [VW_USUARIOS] AS USUARIOS ON (A.CO_MATRICULA_INCLUSAO  = USUARIOS.CO_MATRICULA) 	"
	end if
	...
	If conteudo <> "" then 
		cont = cont + 1
		WHERE(cont)  =  " A.DE_CONTEUDO LIKE '%" & conteudo & "%' "
	End If
	
	If destino <> "" then 
		cont = cont + 1
		WHERE(cont)  =  " A.DE_DESTINO LIKE '%" & destino & "%' "
	End If
 ...
 query = query & " ORDER BY A.DH_ENVIO DESC "
 ' ORDENANDO POR DATA DE TRÁS PRA FRENTE
...
'response.End()
	...
	if not pc.eof then
		...
		Response.Write "<tr>"
		Response.Write "<td width='9%'>N&uacute;mero</td>"
		Response.Write "<td width='8%' >Tipo</td>"
		Response.Write "<td width='7%'>AR</td>"
		Response.Write "<td width='12%'>&Aacute;rea</td>"
		Response.Write "<td width='4%'>Data</td>"
		Response.Write "<td width='17%'>Respons&aacute;vel</td>"
		Response.Write "<td width='23%'>Conte&uacute;do</td>"
		Response.Write "<td width='13%'>Destino</td>"
		if not consultar_base_antiga then	
		...
		do while (not PC.eof and Abs(pc.AbsolutePage - pag) < numPag )
			...
			if not consultar_base_antiga then
				coGS = PC("CO_GS")
				matriculaInclusao = PC("CO_MATRICULA_INCLUSAO")
				matriculaResponsavel = PC("CO_RESPONSAVEL_MATRICULA")
			end if
		
			response.write "<tr>"
			response.write "<td id='numOficio' value="&numeroOficio&"><input type='hidden' id='codSeq' value='"&codSeq&"'/><input type='hidden' id='matriculaInclusao' value='"&matriculaInclusao&"'/><input type='hidden' id='coGS' value='"&coGS&"'/><input type='hidden' id='matriculaResponsavel' value='"&matriculaResponsavel&"'/>" & numeroOficio & " / " & AAProtocolo & "</td>"
			...
			if not consultar_base_antiga then
				response.write "<td >"
				response.write "<img class='edit' id='edit' title='Editar'  src='imagens/edit.png' style='cursor:pointer'> "
				'response.write "</td>"
				'response.write "<td width='1%'>"
				response.write "<img class = 'X' id='X' title='Excluir'  src='imagens/delete_16x16.png' style='cursor:pointer'>"
				response.write "</td>"
			end if
			response.write "</tr>"
