...
if not perfil_acesso_siiag then
	'response.Write("O seu perfil n�o permite o acesso a este m�dulo")
else
	...
	if  (perfil_acesso_admin_siiag or request.Cookies("ic_gerente_siiag") or ucase(request.Cookies("co_usuario_siiag")) = usuario ) then
	...
	%>
         ...
         <form name="form_contato" action="usuario_contatos.asp" method="post" target="_self">
            ...
            <table border="0" style="width:300px; height:auto;" >
            <tr>
            <td>Tipo Contato</td>
            <td>Contato</td>
            <td><img src="imagens\add_16x16.png" title="Adiciona um contato"  style="cursor:pointer"  onClick="linha_salvar.style.display='';" /></td>
            </tr>
            <tr style="display:none;"  id="linha_salvar">
            <td width="230" style='text-align:left;'><%ComboTipoContato()%></td>
        ...
        <%
            do while not rs.eof
                ...
                response.Write "<td>"
        %>
                <img src="imagens\delete_16x16.png" title="Apaga o contato"  style="cursor:pointer" onClick="return(Deletar(<%=rs("CO_CONTATO")%>))"  />
        <%
                response.Write "</td>"
                response.Write "</tr>"	
...
%>
    <select name="f_tipo_contato" id="f_tipo_contato"  class="form-select" style=" width:150px;max-width:150px;min-width:150px;" tabindex="3">
